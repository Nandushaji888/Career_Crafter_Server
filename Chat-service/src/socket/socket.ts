...
io.on("connection", (socket) => {
  ...
  socket.on("message:read:done", async (messageId) => {
    ...
    const message: any = await changeMessageStatusController(
      dependencies,
      messageId
    );
  ...
  socket.on("messageRead", async (messageId) => {
    ...
    const { message }: any = await find_message_controller(
      dependencies,
      messageId
    );
  ...
  socket.on("video:call:room:created", ({ roomName, receiverId }) => {
    ...
    if (receiverSocketId) {
      ...
      io.to(receiverSocketId).emit("video:call:room:created", roomName);
    }
  });
});
