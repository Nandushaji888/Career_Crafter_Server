...
export default (dependencies:any)=> {
    const {useCase:{get_all_posts_usecase}} = dependencies
    ...
    const getAllPostsAdminController = async(req:Request,res:Response)=> {
        try {
            ...
            if(response?.status){
               return res.status(200).json({staus:response?.status,posts:response?.posts})
            }else{
               return res.status(404).json({status:response?.status,message:"File not found"})
            }
        } catch (error) {
